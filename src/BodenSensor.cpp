...
void BodenSensor::computeClosestLineToCenter(const std::vector<int>& activeIndices) {
  std::array<point, 2> bestPair = {};
  double minDist = std::numeric_limits<double>::max();

  if (activeIndices.size() < 1) { // if no sensors active return -1 we can later detect that
    line.dist = -1.0;
    line.rot = 0.0;
  }
  ...
  if (activeIndices.size() == 1) {
    line.dist = 1.0;
    const double x = sensorPositions[activeIndices[0]].x;
    const double y = sensorPositions[activeIndices[0]].y;
    line.rot = std::atan2(y, x) + M_PI_2;
    return;
  }

  for (auto i = 0; i < activeIndices.size(); ++i) {
    for (auto j = i + 1; j < activeIndices.size(); ++j) {
      ...
      const double distance = std::sqrt(mx * mx + my * my);
  ...
  // set line properties
  line.dist = minDist;
  line.rot = std::atan2(bestPair[1].y - bestPair[0].y, bestPair[1].x - bestPair[0].x);
}